...
% 文档信息定义 (根据 guide.tex 保留的信息)
\def \majorTitle   {深度学习课程设计} % 大标题
\def \minorTitleCN {基于双流 CNN-LSTM 融合网络的手语识别} % 中文标题
\def \minorTitleEN {Sign Language Recognition Based on Dual-Stream CNN-LSTM} % 英文标题 (可按需修改)
\def \currentDate  {\zhtoday} % 日期
...
\Header
    {common}
    {1pt}{1pt}{0.5pt}
    {}{\includegraphics[width=0.15\textwidth]{NWPU-title-CN.pdf}}{}
...
\begin{abstractEN}[-2]
    % [待补充：此处填写英文摘要]
    This paper aims to address the dynamic temporal modeling problem in Chinese Sign Language (CSL) recognition. To overcome the limitations of existing datasets, such as simple backgrounds and lack of specificity, a self-built dataset based on daily core vocabularies is constructed. The paper details the hand keypoint extraction pipeline based on MediaPipe, designs an adaptive action segmentation algorithm based on energy thresholds, and proposes a spatio-temporal data augmentation scheme including time warping and Gaussian jittering. Future work will verify the effectiveness of the dataset and preprocessing scheme based on the dual-stream CNN-LSTM network.
...
\imageWatermark{0}{0.8}{0.02}{NWPU-logo.eps}
...
% =========================================================
% 第二章：数据集与预处理 (来自原 main.tex 的内容)
% 注意：原 section 提升为 chapter，subsection 提升为 section
% =========================================================
...
\begin{figure}[H]
    \centering
    \includegraphics[width=1\linewidth]{images/image} % 请确保有此图片或替换为占位符
...
\section{网络总体架构}
[待补充：双流网络（RGB流 + 骨骼流）的整体结构图示与描述...]

\section{空间特征提取模块 (CNN)}
[待补充：CNN 分支的设计，如何处理每一帧的图像特征...]

\section{时序建模模块 (LSTM)}
[待补充：LSTM 单元的结构，如何捕捉长时序依赖...]

\section{融合策略与分类层}
[待补充：特征融合方式（Concat/Sum/Attention）以及 Softmax 分类...]
...
\section{实验环境与参数设置}
[待补充：硬件配置、训练超参数、优化器选择...]

\section{评价指标}
[待补充：Accuracy, Precision, Recall, Confusion Matrix...]

\section{消融实验}
[待补充：对比单流与双流、不同数据增强策略对结果的影响...]
...
\begin{references}
    \bibliography{references.bib} % 请确保目录下有 references.bib 文件
\end{references}
