...
% --- 中文摘要 ---
% 注意：必须在环境开始前定义关键词
\def\keywordsCN{手语识别，双流网络，CNN-LSTM，MediaPipe，数据增强，模糊语义识别}

\begin{abstractCN}[-2]
针对现有手语识别研究中高质量中文数据集匮乏、复杂动作时空特征难以兼顾以及识别结果语义不连贯等问题，本文设计并实现了一个基于双流 CNN-LSTM 融合网络的手语识别系统。 针对特定交互场景，自主构建了包含“好”、“他”、“早上”等核心词汇的手语数据集。利用 MediaPipe 框架提取手部骨骼关键点，并通过能量阈值分割算法实现动作的自动截取；为解决小样本过拟合问题，设计了包含时间扭曲、空间旋转及高斯扰动在内的混合数据增强策略，将数据集扩充至 2862 条。 提出了一种动静结合的双流识别架构。动态流分支利用 CNN 与 Bi-LSTM 级联结构捕捉动作的时序演变轨迹；静态流分支利用全连接网络提取手型空间的几何特征。两者通过特征级融合（Feature Fusion），有效解决了单一模型在面对相似轨迹或微小手型变化时的识别瓶颈。 为了实现从离散词汇到自然语言的跨越，设计了基于模糊语义的后处理模块。通过引入语义组合规则与双重置信度校准机制，成功将识别出的孤立词汇转化为符合中文语法的自然语句。 实验结果表明，该模型在测试集上的识别准确率达到 93.75\%。消融实验证实，双流融合架构相较于单流模型具有显著的性能优势，有效提升了手语识别的鲁棒性与实用性。
\end{abstractCN}
% --- 英文摘要 ---
\def\keywordsEN{Sign Language Recognition, Dual-Stream Network, CNN-LSTM, MediaPipe, Data Augmentation, Fuzzy Semantic Recognition}

\begin{abstractEN}[-2]
To address the challenges in sign language recognition, such as the scarcity of high-quality Chinese datasets, the difficulty in modeling complex spatio-temporal features, and the lack of semantic coherence in recognition results, this paper proposes and implements a sign language recognition system based on a Dual-Stream CNN-LSTM fusion network. Firstly, a specific sign language dataset covering core vocabularies such as ``Good'', ``He'', and ``Morning'' was constructed. MediaPipe was utilized to extract hand skeletal keypoints, and an adaptive energy threshold algorithm was applied for automatic action segmentation. To overcome overfitting caused by small sample sizes, a hybrid data augmentation strategy involving time warping, spatial rotation, and Gaussian jittering was designed, expanding the dataset to 2,862 samples. Secondly, a dual-stream architecture combining dynamic and static features was proposed. The dynamic stream utilizes a cascaded CNN and Bi-LSTM structure to capture the temporal evolution of movements, while the static stream employs a deep neural network to extract spatial geometric features of hand shapes. The feature-level fusion of these two streams effectively resolves recognition bottlenecks encountered by single-stream models when dealing with similar trajectories or subtle hand shape variations.
Furthermore, to bridge the gap between discrete vocabulary and natural language, a post-processing module based on fuzzy semantics was designed. By introducing semantic combination rules and a dual-confidence calibration mechanism, the system successfully converts isolated recognized words into natural sentences that conform to Chinese grammar.
Experimental results show that the model achieves a recognition accuracy of 93.75\% on the test set. Ablation studies confirm that the dual-stream fusion architecture offers significant performance advantages over single-stream models, effectively enhancing the robustness and practicality of sign language recognition.
\end{abstractEN}
% --- 目录 ---
% 参数：行距, 标题名称
\contentPage{1.5}{目~~~~录}
...
\chapter{引言}
\section{研究背景与意义}
手语是听障人士最自然、最核心的沟通工具。然而，由于大多数健听人缺乏手语知识，听障群体在日常交流、教育就业及社会服务等方面面临着巨大的“信息鸿沟”。手语识别技术（Sign Language Recognition, SLR）致力于打破这一壁垒，它充当了听障者与健听者之间的智能翻译官，将肢体语言实时转化为文字或语音。这不仅能显著改善听障人士的生活质量与社交体验，更是推动信息无障碍建设、促进社会公平与包容的重要技术手段，具有极高的人文关怀与社会应用价值。
\section{本文主要工作}
本文针对手语识别在数据获取、时空特征提取以及自然语言转换等方面的挑战，开展了从数据集构建到模型部署的全流程研究。主要工作总结如下：

\begin{enumerate}
    \item \textbf{构建了面向特定交互场景的手语数据集与增强预处理流水线。}针对公开中文手语数据集稀缺的问题，自主采集了包含“好”、“他”、“早上”等核心词汇的视频数据。利用 MediaPipe 框架实现了手部骨骼关键点的高效提取，并提出了基于能量阈值的自适应动作分割算法，实现了对连续视频流中有效动作片段的自动截取。为解决小样本训练导致的过拟合问题，设计了包含时间扭曲（Time Warping）、空间旋转及高斯噪声扰动在内的混合数据增强策略，将原始样本扩充至 2862 条，显著提升了数据的多样性。

    \item \textbf{提出并实现了基于双流 CNN-LSTM 的融合识别网络。}为了同时捕捉手语动作中的“动态轨迹”与“静态手型”特征，设计了一种并行的双流网络架构。其中，动态流分支采用卷积神经网络（CNN）与双向长短时记忆网络（Bi-LSTM）级联，专注于建模动作的时序演变；静态流分支则通过全连接网络提取手部空间几何特征。通过特征级融合（Feature-level Fusion）策略，有效解决了单一模型无法区分相似轨迹或微小手型变化的难题。

    \item \textbf{进行了系统的超参数调优与消融实验验证。}采用控制变量法对学习率、批大小（Batch Size）、LSTM 隐藏层单元数及 Dropout 比率进行了系统性调优，确定了最优参数组合。消融实验结果表明，双流融合架构相比单流模型在准确率、精确率及召回率上均有显著提升。最终模型在测试集上达到了 93.75\% 的识别准确率，且混淆矩阵显示其在易混淆词汇（如“你”与“他”）上具有清晰的决策边界。

    \item \textbf{设计了模糊语义识别与自然语言后处理模块。}针对模型输出的离散孤立词汇难以直接用于自然交流的问题，提出了一套基于语义组合规则的后处理机制。通过序列预处理去除冗余信息，利用语义槽构建上下文，并引入双重置信度校准机制，成功将识别出的关键词转化为符合中文语法习惯的自然语句（如将“早上”转换为“早上好”），实现了从“信号识别”到“语义理解”的跨越。
\end{enumerate}
%=========数据集采集与预处理=======================%
\chapter{数据集获取与预处理}
\label{chap:dataset}
...
我们选用 Adam 优化器，在 Batch Size 固定为 32 的前提下，对比了 0.01、0.001 和 0.0001 三个数量级的学习率。实验结果如表 5-2 所示。
...
Batch Size 决定了单次参数更新所使用的样本数量，直接影响模型梯度的估算准确度与训练的稳定性。考虑到本数据集经过增强后约为 2862 条，数据量属中等规模。我们测试了 16, 32, 64 三种 Batch Size，实验结果如图5-1所示。
...
\begin{figure}[htbp]
    ...
    \begin{minipage}{0.8\linewidth}
        \centering
        \includegraphics[width=\linewidth]{images/Batch_Size-16.png}
        \caption*{BS=16: 最佳 (93.75\%)}
    \end{minipage}
    ...
    \begin{minipage}{0.8\linewidth}
        \centering
        \includegraphics[width=\linewidth]{images/Batch_Size-32.png}
        \caption*{BS=32: 过拟合 (46.05\%)}
    \end{minipage}
    ...
    \begin{minipage}{0.8\linewidth}
        \centering
        \includegraphics[width=\linewidth]{images/Batch_Size-64.png}
        \caption*{BS=64: 泛化差 (61.84\%)}
    \end{minipage}

    \caption{不同 Batch Size 下的模型训练与验证曲线对比}
...
\begin{itemize}
    \item \textbf{Batch Size = 32 与 64}: 从实验曲线可以看出，当 Batch Size 较大时，模型虽然在训练集上迅速收敛（准确率接近 1.0），但在验证集上的表现较差。
    \begin{itemize}
        \item \textbf{Batch Size = 64} 的验证集最高准确率仅为 61.84\%，推测是大批次导致梯度估计过于平滑，使模型陷入了尖锐极小值（Sharp Minima）。
        \item \textbf{Batch Size = 32} 的表现最差，验证集准确率仅为 46.05\%。其 Loss 曲线在早期即出现剧烈震荡并居高不下，表明模型发生了严重的过拟合现象。
    \end{itemize}

    \item \textbf{Batch Size = 16}: 表现最优。虽然小批次会导致单次迭代的梯度估计存在噪声（Noise），但这种随机噪声在训练过程中起到了正则化的作用，帮助模型跳出了质量较差的局部极小值。实验结果显示，Batch Size = 16 在验证集上取得了最高的准确率（93.75\%），且训练曲线与验证曲线的贴合度最好，泛化能力最佳。
\end{itemize}

综上所述，虽然小批次会略微增加训练耗时，但考虑到其带来的显著性能提升，本研究最终选取 \textbf{Batch Size = 16}。
...
\begin{figure}[htbp]
    \centering
    % 请确保图片已放入项目文件夹下的 images/ 目录中
    \includegraphics[width=0.9\linewidth]{images/image5.3.png}
    \caption{消融实验结果对比图}
    \label{fig:training_acc_curve}
\end{figure}
...
\begin{table}[htbp]
    ...
    \begin{tabular}{lcccc}
        ...
        \midrule
        仅静态流 (Static Only) & 69.20\% & 69.2\% & 67.8\% & 0.68 \\
...
\chapter{实时手语交互系统的构建与优化}
\label{ch:realtime_system}

前述章节重点探讨了双流 CNN-LSTM 模型的网络结构设计与离线训练策略。然而，将静态的深度学习模型部署到动态的实时视频流环境中，面临着计算延迟、信号抖动、状态判定模糊等工程挑战。

为了实现“低延迟、高流畅度”的人机交互体验，本章详细阐述了实时手语交互系统的软件架构设计。我们设计了一套包含\textbf{多线程异步 I/O}、\textbf{自适应帧率控制}以及\textbf{指数移动平均滤波}的高效处理管线，并设计了基于状态机的逻辑控制算法，有效解决了动作残留与误触发问题。

\section{系统分层架构设计}
本系统采用模块化分层设计思想，将实时处理流程解耦为五个核心层级，形成即时响应的流水线（Pipeline）。

\begin{figure}[H]
    \centering
    % 请确保 images 文件夹下有名为 image.png/jpg/pdf 的图片，或替换为实际文件名
    % 如果没有图片，请暂时注释掉下面一行以避免编译错误
...
\begin{enumerate}
    \item \textbf{数据获取层（Data Acquisition Layer）：} 负责摄像头的硬件通信与图像帧的原始捕获，采用多线程异步机制消除 I/O 阻塞。
    \item \textbf{预处理与节流层（Preprocessing \& Throttling Layer）：} 执行图像色彩空间转换（BGR 转 RGB），并通过跳帧策略（Frame Skipping）平衡计算负载。
    \item \textbf{特征提取与平滑层（Feature Extraction \& Smoothing Layer）：} 集成 MediaPipe 骨骼检测，并引入指数移动平均（EMA）算法消除高频抖动。
    \item \textbf{时序推理层（Temporal Inference Layer）：} 维护滑动窗口（Sliding Window）队列，驱动双流 CNN-LSTM 模型进行实时概率预测。
    \item \textbf{逻辑控制与交互层（Logic Control \& Interaction Layer）：} 基于有限状态机（FSM）管理动作的开始、结束、冷却与翻译触发，并渲染可视化 UI。
\end{enumerate}

\section{多线程异步 I/O 与计算负载均衡}

\subsection{基于生产者-消费者的异步帧读取}
在传统的单线程视频处理模式中，图像读取（I/O 密集型）与模型推理（计算密集型）串行执行，导致系统帧率（FPS）受限于两者的耗时之和。当模型推理耗时增加时，会造成显著的画面延迟（Latency）。

为解决此问题，本系统实现了基于\textbf{生产者-消费者模型（Producer-Consumer Model）}的 \texttt{WebcamStream} 模块：
\begin{itemize}
    \item \textbf{生产者（子线程）：} 在后台独立线程中以最高帧率持续读取摄像头画面，仅保留最新一帧数据，丢弃陈旧帧。这确保了主程序获取的图像永远是当前物理时刻的最新状态。
    \item \textbf{消费者（主线程）：} 主循环仅负责从内存中拷贝最新帧进行处理，完全解除了对硬件 I/O 的等待依赖。
\end{itemize}

\subsection{自适应跳帧策略 (Frame Skipping)}
考虑到 MediaPipe 关键点检测与双流网络的特征提取具有较高的算力开销，逐帧处理极易导致 CPU 满载。基于人眼视觉暂留特性与手语动作的低频特性（通常动作变化周期 $>200$ms），系统引入了自适应跳帧机制。

设定跳帧间隔 $K=2$，即系统每隔 $K$ 帧执行一次深度特征提取。对于被跳过的中间帧，系统采用\textbf{“采样保持”（Sample-and-Hold）}策略，直接复用上一关键帧的检测结果与骨骼坐标。
\begin{equation}
    R_t =
    \begin{cases}
    \mathcal{F}_{MediaPipe}(I_t), & \text{if } t \mod K = 0 \\
    R_{t-1}, & \text{otherwise}
    \end{cases}
\end{equation}
其中 $I_t$ 为当前帧图像，$R_t$ 为检测结果。实验表明，该策略在几乎不影响识别准确率的前提下，使系统处理帧率提升了约 $80\%$，显著增强了交互的流畅感。

\section{抗抖动与信号平滑处理}
由于光照变化与传感器噪声，原始检测出的手部关键点坐标 $\mathbf{p}_{raw}$ 往往存在高频微小抖动。这种抖动若直接输入 LSTM 网络，会被误判为手部运动特征，导致识别结果不稳定。

本研究在特征提取层引入了\textbf{指数移动平均（Exponential Moving Average, EMA）}滤波器。EMA 是一种一阶低通滤波器，能够有效平滑时间序列数据，同时保留动作的整体趋势。第 $t$ 帧的平滑坐标 $\mathbf{p}_{smooth}^{(t)}$ 计算公式如下：
\begin{equation}
    \mathbf{p}_{smooth}^{(t)} = \alpha \cdot \mathbf{p}_{raw}^{(t)} + (1 - \alpha) \cdot \mathbf{p}_{smooth}^{(t-1)}
\end{equation}
其中，$\alpha \in [0, 1]$ 为平滑因子（Smoothing Factor）。
\begin{itemize}
    \item 当 $\alpha \to 1$ 时，系统响应灵敏但抖动明显。
    \item 当 $\alpha \to 0$ 时，系统极其平稳但存在“拖尾”滞后。
\end{itemize}
经过多次调试，本系统选取 $\alpha = 0.6$，在消除抖动与保持跟手性之间取得了最佳平衡。

\section{基于有限状态机的逻辑控制}

为了处理连续视频流中的动作切分、误触发以及动作间的过渡，系统设计了一个鲁棒的\textbf{有限状态机（Finite State Machine, FSM）}。

\subsection{动作残留问题与缓冲区管理}
在基于滑动窗口的实时识别中，存在经典的“动作残留（Buffer Residual）”问题。即当用户完成动作 A 并撤回手部后，滑动窗口中仍保留了大量动作 A 的历史帧。若用户此时开始动作 B，模型会因窗口中动作 A 的特征占主导而持续误报动作 A，导致无法识别新动作。

为此，系统引入了\textbf{“空闲复位机制”}。定义计数器 $C_{idle}$ 记录未检测到手部的连续帧数：
\begin{equation}
    \text{Action} =
    \begin{cases}
    \text{Reset Buffer}, & \text{if } C_{idle} > \tau_{reset} \\
    \text{Keep Accumulating}, & \text{otherwise}
    \end{cases}
\end{equation}
当 $C_{idle}$ 超过阈值 $\tau_{reset}$（本系统设为 10 帧，约 0.3 秒）时，系统判定当前处于动作间隙，强制清空滑动窗口队列。这确保了每次手部重入画面时，模型都从零开始积累特征，彻底消除了旧动作的干扰。

\subsection{状态流转与冷却机制}
为了避免同一动作被连续重复识别（如长时间保持“好”的手势），系统引入了\textbf{冷却时间（Cooldown）}与\textbf{投票滤波（Voting Filter）}机制。系统主要状态流转逻辑如下：
\begin{enumerate}
    \item \textbf{Accumulating（积累态）：} 手部进入画面，滑动窗口未满，不进行预测。
    \item \textbf{Predicting（预测态）：} 窗口填满，模型逐帧输出概率分布。此时启动\textbf{滑动投票队列}（长度为 5），仅当队列中 $80\%$ 的预测结果一致时，才判定动作有效。
    \item \textbf{Confirmed（确认态）：} 动作被确认识别，将其推入自然语言生成（NLG）模块，并立即进入冷却期。
    \item \textbf{Cooldown（冷却态）：} 在动作触发后的 $1.5$ 秒内，系统锁定输出，忽略新的识别请求，给予用户撤回动作或准备下一动作的时间。
\end{enumerate}

通过上述机制的协同工作，本系统成功实现了从“逐帧识别”到“连续语义交互”的跨越，保证了手语翻译过程的准确性与自然性。

\section{本章小结}
本章从系统工程的角度，详细介绍了实时手语识别系统的构建方案。通过引入多线程异步 I/O 与跳帧策略，解决了深度学习模型在端侧设备上的性能瓶颈；利用 EMA 滤波算法有效提升了关键点数据的信噪比；基于状态机的逻辑控制则赋予了系统处理连续交互的能力。这些优化措施共同构建了一个低延迟、抗干扰、体验丝滑的实时手语翻译原型系统。
