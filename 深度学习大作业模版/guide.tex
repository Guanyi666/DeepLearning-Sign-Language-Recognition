...
\def \minorTitleCN {基于 \LaTeX{} 的报告模板} % 中文标题
\def \minorTitleEN {\LaTeX{}-based Report Template} % 英文标题
...
\def \titlePageInfoBox{
    ...
    \infobox{6cm}{-2}{组~~~~~~~~名}{第X组}\\
    \infobox{6cm}{-2}{姓~~~~~~~~名}{范瑞龙(组长)}\\
    \infobox{6cm}{-2}{学~~~~~~~~号}{XXXXX}\\
...
\section{内容要求}
\begin{itemize}
    \item \textbf{摘要：} 简要描述你的问题、方法和主要结果。字数不应超过300字。
    \item \textbf{简介(10\%：}描述你正在解决的问题、其重要性以及结果概述
    \item \textbf{相关工作 (10\%)：} 讨论与你的项目相关的已发表的工作。你的方法与其他人有何相似或不同？
    \item \textbf{方法 (30\%)：}讨论解决引言中提出的问题的方法。为什么你的方法是正确的？你是否考虑过其他方法？你应该证明你已应用本课程学习到的方法和技能来解决你选择的问题。使用图形、图表或表格来描述你的方法或将其与其他方法进行比较可能会有所帮助。
    \item \textbf{数据 (10\%)：}描述你的项目正在使用的数据。它是什么类型的数据？它从哪里来的？你使用了多少数据？你是否需要进行任何预处理、过滤或其他特殊处理才能在项目中使用这些数据？（要求有自己收集部分）
    \item \textbf{实验 (30\%)：} 为证明你的方法可以解决问题而进行的实验。需要与以前的方法进行比较，尝试不同的超参数或架构选择，需要尝试不同调参方法，执行消融研究以确定系统各个组件的影响，使用可视化技术来深入了解如何你的模型是否有效，讨论模型的常见故障模式等。你应该包括图表、表格或其他图形来说明你的实验结果。
    \item \textbf{结论 (10\%):} 总结你的主要结果 - 你学到了什么？为你的想法的未来扩展或新应用提出建议。
